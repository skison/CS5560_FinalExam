...
export class StocksPage implements OnInit, AfterViewInit {
  ...
  constructor(element: ElementRef, d3Service: D3Service,
              ...
              { 
               ...
               /* this.platformHeight = platform.height(); //get platform's width and height
  ...
  drawBarChart()
  {
    ...
    if (this.parentNativeElement !== null) {
      ...
      /*if(this.parentNativeElement.children[3] == null){return;}*/
      d3ParentElement = d3.select(this.parentNativeElement);
      //d3ParentElement = d3.select(this.parentNativeElement);
      d3Svg = this.d3Svg = d3ParentElement.select<SVGSVGElement>('#barchart');

      /*if(d3Svg == null){return;}
      console.log("Svg element: ");
      console.log(d3Svg['_groups'][0]);*/

      if(d3Svg['_groups'][0] == ''){return;} /*prevent rendering if element isn't loaded yet*/
  ...
  deleteStock(stock: Stock) {
    let confirm = this.alertCtrl.create({
      title: 'Delete Stock?',
      message: 'Are you sure you want to permanently delete ' + stock.stocksymbol + '?',
      buttons: [
        {
          text: 'No',
          handler: () => {
